...
abstract class Collector
{
    ...
    public function Init(): void
    {
        ...
        foreach ($aSourceDefinition['attribute_list'] as $aAttr) {
            $this->aFields[$aAttr['attcode']] = ['class' => $aAttr['finalclass'], 'update' => (0 != $aAttr['update']), 'reconcile' => (0 != $aAttr['reconcile'])];
        }
        ...
        if (array_key_exists('nullified_attributes', $this->aCollectorConfig)) {
            ...
            if (null === $this->aNullifiedAttributes) {
                // Try all lowercase
    ...
    public function GetSynchroDataSourceDefinition($aPlaceHolders = [])
    {
        ...
        if (false === $this->sSynchroDataSourceDefinitionFile) {
            return false;
        }
    ...
    protected function DoProcessBeforeSynchro()
    {
        ...
        foreach ($aFiles as $sDataFile) {
            ...
            if (preg_match($sPattern, $sDataFile, $aMatches)) {
                ...
                if ((false !== $hCSV) && (false !== $hOutputCSV)) {
                    $iLineIndex = 0;
    ...
    protected function CheckDataSourceDefinition($aExpectedSourceDefinition)
    {
        ...
        if ('use_attributes' == $aExpectedSourceDefinition['reconciliation_policy']) {
            $bReconciliationKeyFound = false;
            foreach ($aExpectedSourceDefinition['attribute_list'] as $aAttributeDef) {
                if ('1' == $aAttributeDef['reconcile']) {
                    $bReconciliationKeyFound = true;
                    break;
                }
            }
            if (!$bReconciliationKeyFound) {
                throw new InvalidConfigException("Collector::CheckDataSourceDefinition: Missing reconciliation key for data source '{$aExpectedSourceDefinition['name']}'. At least one attribute in 'attribute_list' must have the flag 'reconcile' set to '1'.");
            }
        }
        ...
        if (!preg_match(self::TABLENAME_PATTERN, $sDatabaseTableName)) {
            throw new InvalidConfigException("Collector::CheckDataSourceDefinition: '{$aExpectedSourceDefinition['name']}' invalid characters in database_table_name, current value is '$sDatabaseTableName'");
        }
    ...
    public function InitSynchroDataSource($aPlaceholders)
    {
        ...
        try {
            ...
            if (0 != $aResult['code']) {
                ...
                $iCount = (null !== $aResult['objects']) ? count($aResult['objects']) : 0;
                switch ($iCount) {
                    case 0:
        ...
        try {
            ...
            if ($bResult) {
                ...
                while ($aRow = $this->Fetch()) {
                    ...
                    if ((0 == $idx) || (($iMaxChunkSize > 0) && (($idx % $iMaxChunkSize) == 0))) {
                        $this->NextCSVFile();
    ...
    protected function AddHeader($aHeaders)
    {
        ...
        foreach ($aHeaders as $sHeader) {
            if (('primary_key' != $sHeader) && !array_key_exists($sHeader, $this->aFields)) {
    ...
    protected function OpenCSVFile()
    {
        ...
        if (false === $this->aCSVFile[$this->iFileIndex]) {
    ...
    protected function UpdateSDSAttributes($aExpectedAttrDef, $aSynchroAttrDef, $sComment, ?RestClient $oClient = null)
    {
        ...
        foreach ($aSynchroAttrDef as $aAttr) {
            ...
            if ($aAttr != $aExpectedAttr) {
                if ((false === $aExpectedAttr) && $this->AttributeIsOptional($aAttr['attcode'])) {
                    ...
                    // Fix booleans
                    $aAttr['update'] = (1 == $aAttr['update']) ? '1' : '0';
                    $aAttr['reconcile'] = (1 == $aAttr['reconcile']) ? '1' : '0';
